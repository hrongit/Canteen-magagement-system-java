...
public class UserInterface_GUI extends JFrame implements ActionListener {
    ...
    public UserInterface_GUI(Controller_GUI rController) {
        ...
        // Set frame
        setTitle("34 Block Canteen Management System");
    ...
    private void createMasterPanelConpornents() {
        ...
        headTitle = new JLabel("34 Block Canteen Management System");
    ...
    private class MenuListPanel extends JPanel implements ActionListener {
        ...
        public MenuListPanel() {
            ...
            btnPanel.add(btnJUICE);
            btnPanel.add(btnCOLA);
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            } else if (ae.getSource() == btnJUICE) {
                showMenuList(MenuItem.JUICE);
                // showParticularMenuList(MenuItem.JUICE, displayArea);
            } else if (ae.getSource() == btnCOLA) {
                showMenuList(MenuItem.COLA);
                // showParticularMenuList(MenuItem.COLA, displayArea);
    ...
    private class EditMenuItemPanel extends JPanel implements ActionListener {
        ...
        public EditMenuItemPanel() {
            ...
            String[] combodata = { "Main", "Juice", "Cola", "Dessert" };
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == btnOK) {
                ...
                } else if (strMenuType.equals("Juice")) {
                    menuType = MenuItem.JUICE;
                } else if (strMenuType.equals("Cola")) {
                    menuType = MenuItem.COLA;
                } else // Dessert
                {
                ...
                if (isUpdate) {
                    if (!rcController.updateMenuItem(menuItemID, tbName.getText(),
                            Double.parseDouble(tbPrice.getText()), menuType)) {
                        showErrorDialog("Error", rcController.getErrorMessage());
                        return;
                    }
                    showConfirmDialog("Message", "Update successful!!");
                } else {
                    if (!rcController.addNewMenuItem(menuItemID, tbName.getText(),
                            Double.parseDouble(tbPrice.getText()), menuType)) {
                        showErrorDialog("Error", rcController.getErrorMessage());
                        return;
                    }
    ...
    private class EditEmployeePanel extends JPanel implements ActionListener {
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == btnOK) {
                ...
                if (tbPassword.getPassword().length == 0 || tbFirstName.getText().equals("")
                ...
                if (isUpdate) {
                    if (!rcController.updateStaff(staffID, new String(tbPassword.getPassword()), tbFirstName.getText(),
                            tbLastName.getText())) {
                        showErrorDialog("Error", rcController.getErrorMessage());
                        return;
                    }
    ...
    private class OrderDetailPanel extends JPanel implements ActionListener, ListSelectionListener {
        ...
        public OrderDetailPanel() {
            ...
            // menuScrollPanel.setMaximumSize(new Dimension(Short.MAX_VALUE,
            // Short.MAX_VALUE));
            gbc.gridy = 1;
            gbc.weighty = 1.0;
            ...
            gbc.gridx = 2;
            gbc.gridy = 2;
            gbLayout.setConstraints(btnJUICE, gbc);
            menuListPanel.add(btnJUICE);

            btnCOLA = new JButton("COLA");
            btnCOLA.addActionListener(this);
            gbc.gridx = 3;
            gbc.gridy = 2;
            gbLayout.setConstraints(btnCOLA, gbc);
            menuListPanel.add(btnCOLA);
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == btnAddItem) {
                ...
                // createParticularMenuList(MenuItem.JUICE, menuList);
                menuList.setListData(rcController.createMenuList(MenuItem.JUICE).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            } else if (ae.getSource() == btnCOLA) {
                // createParticularMenuList(MenuItem.COLA, menuList);
                menuList.setListData(rcController.createMenuList(MenuItem.COLA).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            } else if (ae.getSource() == btnDessert) {
